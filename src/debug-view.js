...
export function renderDebug(view) {
  ...
  view.innerHTML = `
    ...
    <label class="backend-switch">
      ...
      <span class="bs-opt ${debug ? 'active' : ''}">Debug data 🧪</span>
  ...
  view.querySelector('#d-toggle-backend').addEventListener('change', async () => {
    ...
    rerender(`Switching to ${toDebug ? 'DEBUG' : 'PROD'}…`) // reveal/hide debug tools now
    await pull()
    rerender(
  ...
  // --- Debug tools: only present in debug-backend mode ---
  if (debug) {
    on('#d-seed', async () => {
      const n = seed(21)
    ...
    on('#d-del-backend', async () => {
      const synced = getEntries().find((e) => e.synced)
      if (!synced) {
